> ✅ **HOÀN THÀNH (ARCHIVED)** — Kế hoạch tái cấu trúc bên dưới đã được thực hiện xong: `workers/`, `ui/`, `hooks/`, `utils/` đã tách khỏi `__init__.py`. Giữ file này làm lịch sử thiết kế; không dùng làm tài liệu hiện trạng. Xem cấu trúc thật tại [`.claude/skills/01-project-map/SKILL.md`](.claude/skills/01-project-map/SKILL.md).
