...
from .color_widgets import ModernColorPickerDialog
from .dialogs import DonationDialog
from .pickers import IconPickerDialog, ThemeCardWidget
from .pixmap_utils import create_rounded_pixmap
from .search import SettingsSearchPage
from .widgets import AnimatedToggleButton, BirthdayWidget, CircularColorButton, ColorSwatch, FlowLayout, FontCardWidget, ProfileBarWidget, SectionGroup, SelectionOverlay, SidebarToggleButton, ThumbnailWorker
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None, addon_path=None, initial_page_index=0):
        ...
        self.setWindowTitle("Bento Station Settings")
        self.setWindowTitle("Bento Station Settings")
        ...
        self.pages = {
            ...
            "Study Tools": self.create_study_tools_page,
        }
        ...
        menu_items = ["Main menu", "Sidebar", "Study Tools"]
    ...
    # --- Habit Engine: Streak Insurance helpers (GĐ3) ---
    def _refresh_streak_tokens_label(self):
        try:
            from . import habit_engine
            tokens = habit_engine.get_streak_tokens()
            self.streak_tokens_label.setText(f"🛡️ Current tokens: {tokens}")
        except Exception:
            self.streak_tokens_label.setText("🛡️ Current tokens: 0")

    def _reset_streak_tokens_clicked(self):
        try:
            from . import habit_engine
            habit_engine.reset_streak_tokens()
            self._refresh_streak_tokens_label()
        except Exception:
            pass
    ...
    class OnigiriDraggableItem(DraggableItem):
        ...
        def contextMenuEvent(self, event):
            ...
            height_group.setExclusive(True)
            # Set row constraints
            min_rows = 2 if is_heatmap else 1
            if is_heatmap:
    ...
    class UnifiedLayoutEditor(QWidget):
        ...
        _WIDGET_DEFINITIONS = {
            "studied": "Studied Card", "time": "Time Card",
            "pace": "Pace Card", "retention": "Retention Card", "heatmap": "Heatmap",
            "favorites": "Favorites Widget", "level": "EXP Level Bar",
            "planner": "Study Planner", "timer": "Focus Timer",
            "lookup": "Quick Lookup", "gallery": "Flashcard Gallery",
        }
    ...
    def create_main_menu_page(self):
        ...
        # --- Effort Mode (Habit Engine GĐ1) ---
        effort_section, effort_layout = self._create_inner_group("Effort Mode")
        self.heatmap_effort_mode_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.heatmap_effort_mode_check.setChecked(self.current_config.get("heatmapEffortMode", False))
        effort_layout.addWidget(self._create_toggle_row(
            self.heatmap_effort_mode_check,
            "Color by real effort (time + correctness + difficulty) instead of card count"
        ))

        effort_weights = self.current_config.get("heatmapEffortWeights", {"time": 1.0, "correct": 1.0, "difficulty": 0.5})
        self.heatmap_weight_time_spin = QSpinBox()
        self.heatmap_weight_correct_spin = QSpinBox()
        self.heatmap_weight_diff_spin = QSpinBox()
        for spin, label, val in [
            (self.heatmap_weight_time_spin, "Time weight", effort_weights.get("time", 1.0)),
            (self.heatmap_weight_correct_spin, "Correctness weight", effort_weights.get("correct", 1.0)),
            (self.heatmap_weight_diff_spin, "Difficulty weight", effort_weights.get("difficulty", 0.5)),
        ]:
            spin.setRange(0, 200)
            spin.setValue(int(round(val * 100)))
            spin.setSuffix(" %")
            weight_row = QWidget()
            weight_row_layout = QHBoxLayout(weight_row)
            weight_row_layout.setContentsMargins(0, 0, 0, 0)
            weight_row_layout.addWidget(QLabel(label))
            weight_row_layout.addStretch()
            weight_row_layout.addWidget(spin)
            effort_layout.addWidget(weight_row)

        heatmap_section.add_widget(effort_section)
        ...
        # ═══════════════════════════════════════════════
        # 🎌 Habit Engine Section (GĐ2+)
        # ═══════════════════════════════════════════════
        habit_section = SectionGroup(
            "Habit Engine",
            self,
            border=False,
            description="Tính năng duy trì thói quen: Suggested Session, Streak Insurance, Future Self Letter."
        )

        session_section, session_layout = self._create_inner_group("Suggested Session")
        self.session_suggest_enabled_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.session_suggest_enabled_check.setChecked(self.current_config.get("sessionSuggestEnabled", True))
        session_layout.addWidget(self._create_toggle_row(
            self.session_suggest_enabled_check,
            "Suggest a daily card target based on your pace and mood (anti-burnout)"
        ))
        habit_section.add_widget(session_section)

        streak_section, streak_layout = self._create_inner_group("Streak Insurance")
        self.streak_insurance_enabled_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.streak_insurance_enabled_check.setChecked(self.current_config.get("streakInsuranceEnabled", True))
        streak_layout.addWidget(self._create_toggle_row(
            self.streak_insurance_enabled_check,
            "Earn tokens by exceeding your daily average; tokens auto-cover a missed day"
        ))
        self.streak_tokens_label = QLabel("")
        streak_layout.addWidget(self.streak_tokens_label)
        self._refresh_streak_tokens_label()
        reset_tokens_button = QPushButton("Reset Tokens")
        reset_tokens_button.clicked.connect(self._reset_streak_tokens_clicked)
        streak_layout.addWidget(reset_tokens_button)
        habit_section.add_widget(streak_section)

        letter_section, letter_layout = self._create_inner_group("Future Self Letter")
        self.future_letter_enabled_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.future_letter_enabled_check.setChecked(self.current_config.get("futureLetterEnabled", True))
        letter_layout.addWidget(self._create_toggle_row(
            self.future_letter_enabled_check,
            "Write letters at milestones and receive them at the next one"
        ))
        habit_section.add_widget(letter_section)

        layout.addWidget(habit_section)

        # ═══════════════════════════════════════════════
        # 🎌 EXP Level Bar Section
        # ═══════════════════════════════════════════════
        exp_bar_section = SectionGroup(
            "EXP Level Bar",
            self,
            border=False,
            description="Customize the RPG-style EXP level bar below the heatmap."
        )

        # Enable/Disable toggle
        self.exp_bar_enabled_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.exp_bar_enabled_check.setChecked(self.current_config.get("expBarEnabled", True))
        exp_bar_section.add_widget(self._create_toggle_row(self.exp_bar_enabled_check, "Show EXP Level Bar"))

        # Text customization
        text_section, text_layout = self._create_inner_group("Text Labels")
        form_layout = QFormLayout()
        form_layout.setFieldGrowthPolicy(QFormLayout.FieldGrowthPolicy.ExpandingFieldsGrow)
        self.exp_bar_title_input = QLineEdit(self.current_config.get("expBarTitle", "Bento Station Mastery"))
        form_layout.addRow("Title:", self.exp_bar_title_input)
        self.exp_bar_lv_label_input = QLineEdit(self.current_config.get("expBarLevelLabel", "LV"))
        form_layout.addRow("Level prefix:", self.exp_bar_lv_label_input)
        self.exp_bar_next_text_input = QLineEdit(self.current_config.get("expBarNextLevelText", "Next level:"))
        form_layout.addRow("'Next level' text:", self.exp_bar_next_text_input)
        text_layout.addLayout(form_layout)
        exp_bar_section.add_widget(text_section)

        # Visibility toggles
        vis_section, vis_layout = self._create_inner_group("Display Options")
        self.exp_bar_show_remaining_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.exp_bar_show_remaining_check.setChecked(self.current_config.get("expBarShowRemaining", True))
        vis_layout.addWidget(self._create_toggle_row(self.exp_bar_show_remaining_check, "Show 'Next level: X EXP' text"))
        self.exp_bar_show_percent_check = AnimatedToggleButton(accent_color=self.accent_color)
        self.exp_bar_show_percent_check.setChecked(self.current_config.get("expBarShowPercent", True))
        vis_layout.addWidget(self._create_toggle_row(self.exp_bar_show_percent_check, "Show percentage"))
        exp_bar_section.add_widget(vis_section)

        layout.addWidget(exp_bar_section)

        divider_exp = QFrame()
        divider_exp.setFrameShape(QFrame.Shape.HLine)
        divider_exp.setFrameShadow(QFrame.Shadow.Sunken)
        layout.addWidget(divider_exp)
    ...
    def _update_icon_preview_for_widget(self, widget, size=24):
        ...
        if filename:
            ...
            if filename.startswith("registry:"):
                reg_name = filename[9:]
                svg_xml = icon_registry.get_icon_svg(reg_name) or ""
            else:
        ...
        if not svg_xml:
            # 🎌 Try icon registry via SYSTEM_ICON_MAP
            reg_name = icon_registry.SYSTEM_ICON_MAP.get(key, key)
            svg_xml = icon_registry.get_icon_svg(reg_name) or ""
            if not svg_xml:
    ...
    def _save_main_menu_settings(self):
        ...
        # --- Effort Mode (Habit Engine GĐ1) ---
        if hasattr(self, "heatmap_effort_mode_check"):
            self.current_config["heatmapEffortMode"] = self.heatmap_effort_mode_check.isChecked()
        if hasattr(self, "heatmap_weight_time_spin"):
            self.current_config["heatmapEffortWeights"] = {
                "time": self.heatmap_weight_time_spin.value() / 100.0,
                "correct": self.heatmap_weight_correct_spin.value() / 100.0,
                "difficulty": self.heatmap_weight_diff_spin.value() / 100.0,
            }

        # --- Suggested Session (Habit Engine GĐ2) ---
        if hasattr(self, "session_suggest_enabled_check"):
            self.current_config["sessionSuggestEnabled"] = self.session_suggest_enabled_check.isChecked()

        # --- Streak Insurance (Habit Engine GĐ3) ---
        if hasattr(self, "streak_insurance_enabled_check"):
            self.current_config["streakInsuranceEnabled"] = self.streak_insurance_enabled_check.isChecked()

        # --- Future Self Letter (Habit Engine GĐ4) ---
        if hasattr(self, "future_letter_enabled_check"):
            self.current_config["futureLetterEnabled"] = self.future_letter_enabled_check.isChecked()
        ...
        # --- Save EXP Level Bar Settings ---
        if hasattr(self, "exp_bar_enabled_check"):
            self.current_config["expBarEnabled"] = self.exp_bar_enabled_check.isChecked()
        if hasattr(self, "exp_bar_title_input"):
            self.current_config["expBarTitle"] = self.exp_bar_title_input.text() or "Bento Station Mastery"
        if hasattr(self, "exp_bar_lv_label_input"):
            self.current_config["expBarLevelLabel"] = self.exp_bar_lv_label_input.text() or "LV"
        if hasattr(self, "exp_bar_next_text_input"):
            self.current_config["expBarNextLevelText"] = self.exp_bar_next_text_input.text() or "Next level:"
        if hasattr(self, "exp_bar_show_remaining_check"):
            self.current_config["expBarShowRemaining"] = self.exp_bar_show_remaining_check.isChecked()
        if hasattr(self, "exp_bar_show_percent_check"):
            self.current_config["expBarShowPercent"] = self.exp_bar_show_percent_check.isChecked()
    ...
    def create_study_tools_page(self):
        """Trang tùy chỉnh sâu cho Study Tools (4 công cụ học tập)."""
        cfg = self.current_config.get("studyTools") or {}
        page, layout = self._create_scrollable_page()
        ...
        master = SectionGroup(
            ...
            self,
            border=False,
            description="Bật/tắt toàn bộ bộ công cụ và nút trên Sidebar + Menu 'Bento Station'.",
        )
        self.st_enabled_cb = QCheckBox()
        self.st_enabled_cb.setChecked(bool(cfg.get("enabled", True)))
        master.add_widget(self._create_toggle_row(self.st_enabled_cb, "Bật Study Tools"))
        self.st_sidebar_cb = QCheckBox()
        self.st_sidebar_cb.setChecked(bool(cfg.get("sidebar_button", True)))
        master.add_widget(self._create_toggle_row(self.st_sidebar_cb, "Hiển thị nút 'Study Tools' trên Sidebar"))
        layout.addWidget(master)

        # ── 1) Study Planner ──
        p = cfg.get("planner") or {}
        planner_section = SectionGroup(
            "📅 Study Planner",
            self,
            description="Lịch/mục tiêu học hằng ngày + theo dõi tiến độ trên Dashboard.",
        )
        self.st_planner_enabled_cb = QCheckBox()
        self.st_planner_enabled_cb.setChecked(bool(p.get("enabled", True)))
        planner_section.add_widget(self._create_toggle_row(self.st_planner_enabled_cb, "Bật Study Planner"))
        self.st_planner_daily = QSpinBox()
        self.st_planner_daily.setRange(1, 10000)
        self.st_planner_daily.setSuffix(" thẻ/ngày")
        self.st_planner_daily.setValue(int(p.get("daily_goal_cards", 50)))
        planner_section.add_widget(self._create_toggle_row(self.st_planner_daily, "Mục tiêu hằng ngày:"))
        self.st_planner_weekly = QSpinBox()
        self.st_planner_weekly.setRange(1, 100000)
        self.st_planner_weekly.setSuffix(" thẻ/tuần")
        self.st_planner_weekly.setValue(int(p.get("weekly_goal_cards", 350)))
        planner_section.add_widget(self._create_toggle_row(self.st_planner_weekly, "Mục tiêu hằng tuần:"))
        self.st_planner_widget_cb = QCheckBox()
        self.st_planner_widget_cb.setChecked(bool(p.get("show_dashboard_widget", True)))
        planner_section.add_widget(self._create_toggle_row(self.st_planner_widget_cb, "Widget Study Planner trên Dashboard"))
        layout.addWidget(planner_section)

        # ── 2) Quick Lookup ──
        q = cfg.get("quick_lookup") or {}
        lookup_section = SectionGroup(
            "🔍 Quick Lookup",
            self,
            description="Tra từ nhanh → mở web / tìm trong Anki / đúc thẻ Bento Forge 1 chạm.",
        )
        self.st_lookup_enabled_cb = QCheckBox()
        self.st_lookup_enabled_cb.setChecked(bool(q.get("enabled", True)))
        lookup_section.add_widget(self._create_toggle_row(self.st_lookup_enabled_cb, "Bật Quick Lookup"))
        self.st_lookup_url = QLineEdit(str(q.get("dictionary_url") or "https://jisho.org/search/{word}"))
        self.st_lookup_url.setPlaceholderText("URL tra từ — dùng {word} cho từ cần tra")
        self.st_lookup_url.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        lookup_section.add_widget(self._create_toggle_row(self.st_lookup_url, "URL tra cứu web:"))
        self.st_lookup_browser_cb = QCheckBox()
        self.st_lookup_browser_cb.setChecked(bool(q.get("open_browser_on_lookup", True)))
        lookup_section.add_widget(self._create_toggle_row(self.st_lookup_browser_cb, "Tự đóng sau khi mở web"))
        layout.addWidget(lookup_section)

        # ── 3) Flashcard Gallery ──
        g = cfg.get("gallery") or {}
        gallery_section = SectionGroup(
            "🗂️ Flashcard Gallery",
            self,
            description="Duyệt / ôn tập thẻ đã có trong bộ sưu tập (lọc theo truy vấn Anki).",
        )
        self.st_gallery_enabled_cb = QCheckBox()
        self.st_gallery_enabled_cb.setChecked(bool(g.get("enabled", True)))
        gallery_section.add_widget(self._create_toggle_row(self.st_gallery_enabled_cb, "Bật Flashcard Gallery"))
        self.st_gallery_per_page = QSpinBox()
        self.st_gallery_per_page.setRange(10, 2000)
        self.st_gallery_per_page.setSuffix(" thẻ/lần tải")
        self.st_gallery_per_page.setValue(int(g.get("per_page", 200)))
        gallery_section.add_widget(self._create_toggle_row(self.st_gallery_per_page, "Giới hạn thẻ:"))
        self.st_gallery_query = QLineEdit(str(g.get("default_query") or "deck:*"))
        self.st_gallery_query.setPlaceholderText("Truy vấn mặc định (VD: deck:* tag:vocab)")
        self.st_gallery_query.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed)
        gallery_section.add_widget(self._create_toggle_row(self.st_gallery_query, "Truy vấn mặc định:"))
        layout.addWidget(gallery_section)

        # ── 4) Focus Timer ──
        ft = cfg.get("focus_timer") or {}
        timer_section = SectionGroup(
            "⏱️ Focus Timer (Pomodoro)",
            self,
            description="Đồng hồ tập trung: chu kỳ tập trung / nghỉ ngắn / nghỉ dài.",
        )
        self.st_timer_enabled_cb = QCheckBox()
        self.st_timer_enabled_cb.setChecked(bool(ft.get("enabled", True)))
        timer_section.add_widget(self._create_toggle_row(self.st_timer_enabled_cb, "Bật Focus Timer"))
        self.st_timer_work = QSpinBox()
        self.st_timer_work.setRange(1, 120)
        self.st_timer_work.setSuffix(" phút")
        self.st_timer_work.setValue(int(ft.get("work_min", 25)))
        timer_section.add_widget(self._create_toggle_row(self.st_timer_work, "Thời gian tập trung:"))
        self.st_timer_break = QSpinBox()
        self.st_timer_break.setRange(1, 60)
        self.st_timer_break.setSuffix(" phút")
        self.st_timer_break.setValue(int(ft.get("break_min", 5)))
        timer_section.add_widget(self._create_toggle_row(self.st_timer_break, "Nghỉ ngắn:"))
        self.st_timer_long = QSpinBox()
        self.st_timer_long.setRange(1, 90)
        self.st_timer_long.setSuffix(" phút")
        self.st_timer_long.setValue(int(ft.get("long_break_min", 15)))
        timer_section.add_widget(self._create_toggle_row(self.st_timer_long, "Nghỉ dài:"))
        self.st_timer_sessions = QSpinBox()
        self.st_timer_sessions.setRange(1, 12)
        self.st_timer_sessions.setSuffix(" phiên")
        self.st_timer_sessions.setValue(int(ft.get("sessions_before_long", 4)))
        timer_section.add_widget(self._create_toggle_row(self.st_timer_sessions, "Số phiên trước nghỉ dài:"))
        self.st_timer_auto = QCheckBox()
        self.st_timer_auto.setChecked(bool(ft.get("auto_start_break", True)))
        timer_section.add_widget(self._create_toggle_row(self.st_timer_auto, "Tự chuyển sang nghỉ khi hết phiên"))
        layout.addWidget(timer_section)

        layout.addStretch()
        return page

    def _save_study_tools_settings(self):
        """Lưu cấu hình Study Tools từ trang Settings."""
        cfg = dict(self.current_config.get("studyTools") or {})
        cfg["enabled"] = self.st_enabled_cb.isChecked()
        cfg["sidebar_button"] = self.st_sidebar_cb.isChecked()

        cfg.setdefault("planner", {})
        cfg["planner"]["enabled"] = self.st_planner_enabled_cb.isChecked()
        cfg["planner"]["daily_goal_cards"] = self.st_planner_daily.value()
        cfg["planner"]["weekly_goal_cards"] = self.st_planner_weekly.value()
        cfg["planner"]["show_dashboard_widget"] = self.st_planner_widget_cb.isChecked()

        cfg.setdefault("quick_lookup", {})
        cfg["quick_lookup"]["enabled"] = self.st_lookup_enabled_cb.isChecked()
        cfg["quick_lookup"]["dictionary_url"] = self.st_lookup_url.text().strip() or "https://jisho.org/search/{word}"
        cfg["quick_lookup"]["open_browser_on_lookup"] = self.st_lookup_browser_cb.isChecked()

        cfg.setdefault("gallery", {})
        cfg["gallery"]["enabled"] = self.st_gallery_enabled_cb.isChecked()
        cfg["gallery"]["per_page"] = self.st_gallery_per_page.value()
        cfg["gallery"]["default_query"] = self.st_gallery_query.text().strip() or "deck:*"

        cfg.setdefault("focus_timer", {})
        cfg["focus_timer"]["enabled"] = self.st_timer_enabled_cb.isChecked()
        cfg["focus_timer"]["work_min"] = self.st_timer_work.value()
        cfg["focus_timer"]["break_min"] = self.st_timer_break.value()
        cfg["focus_timer"]["long_break_min"] = self.st_timer_long.value()
        cfg["focus_timer"]["sessions_before_long"] = self.st_timer_sessions.value()
        cfg["focus_timer"]["auto_start_break"] = self.st_timer_auto.isChecked()

        self.current_config["studyTools"] = cfg
    ...
    def save_settings(self):
        ...
        if self.tabs_loaded.get(page_indices.get("Study Tools")):
            self._save_study_tools_settings()
        ...
        # <<< FIX: Đánh dấu collection "dirty" để mw.col.conf (nền, sidebar, modern_menu_*, ...)
        #         được flush vào collection khi thoát Anki — tránh giao diện Deck Browser
        #         quay về mặc định ở lần khởi động sau. >>>
        try:
            mw.col.setMod()
        except Exception:
            pass
