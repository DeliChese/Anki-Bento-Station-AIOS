...
(function () {
    ...
    function setupResizeHandle() {
        ...
        const updateSidebarWidth = () => {
            ...
            // REVERSED: Handle is on LEFT edge of RIGHT sidebar
            // Drag RIGHT (positive deltaX) → sidebar SHRINKS
            // Drag LEFT (negative deltaX) → sidebar EXPANDS
        ...
        // Throttled handle indicator - only update every 2nd frame
        let indicatorFrameSkip = 0;
        const mousemoveHandler = (e) => {
            if (isResizing) return;
            indicatorFrameSkip++;
            if (indicatorFrameSkip % 2 !== 0) return;
        ...
        const mousedownHandler = (e) => {
            ...
            startWidth = sidebarEl.offsetWidth; // Single forced read, cache it
        ...
        const documentMouseupHandler = () => {
            if (isResizing) {
                ...
                // Release GPU layer after resize
        ...
        handle.addEventListener('mousemove', mousemoveHandler, { passive: true });
        handle.addEventListener('mouseleave', mouseleaveHandler, { passive: true });
        handle.addEventListener('mousedown', mousedownHandler);
        document.addEventListener('mousemove', documentMousemoveHandler, { passive: true });
        document.addEventListener('mouseup', documentMouseupHandler, { passive: true });
    }
