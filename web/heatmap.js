...
(function (exports) {
    ...
    function prepareData(rawData) {
        ...
        // Effort score map (Habit Engine GĐ1): dateKey -> {score, count, time_ms, correct, ...}
        const effortByDay = new Map(Object.entries(rawData.effort_by_day || {}));
        ...
        return { reviewsByDay, duesByDay, effortByDay, todayKey, dailyAverage };
    }
    ...
    // Maps an effort score (0-100) to an intensity level (0-8) for effort mode
    function getEffortIntensityLevel(score) {
        if (!score) return 0;
        return Math.max(0, Math.min(8, Math.round(score / 12.5)));
    }

    // Builds a detailed tooltip for a day in effort mode
    function buildEffortTooltip(effort, isToday, dateText) {
        const mins = Math.round((effort.time_ms || 0) / 60000);
        const timeStr = mins >= 60 ? Math.floor(mins / 60) + 'h ' + (mins % 60) + 'm' : mins + 'm';
        const pct = Math.round((effort.correct_ratio || 0) * 100);
        const diff = effort.difficulty >= 0.66 ? 'Hard' : effort.difficulty >= 0.33 ? 'Medium' : 'Easy';
        const base = `Effort ${effort.score}/100 · ${timeStr} · ${pct}% correct · ${diff}`;
        return isToday ? `${base} today` : `${base} · ${dateText}`;
    }
    ...
    function createCell(date, reviewCount, dueCount, config, todayKey, dailyAverage) {
        ...
        const effortData = state.preparedData && state.preparedData.effortByDay
            ? state.preparedData.effortByDay.get(dateKey) || null
            : null;
        const useEffort = (mode === 'effort' && effortData);
        let tooltipText;
        ...
        if (dateKey === todayKey) {
            ...
            cell.dataset.level = useEffort
                ? getEffortIntensityLevel(effortData.score)
                : getIntensityLevel(reviewCount, dailyAverage);

            // Build tooltip with reviews done today
            tooltipText = useEffort
                ? buildEffortTooltip(effortData, true, dateText)
                : `${reviewCount} review${reviewCount !== 1 ? 's' : ''} done today`;
            ...
            cell.dataset.level = useEffort
                ...
                : getIntensityLevel(reviewCount, dailyAverage);
            tooltipText = useEffort
                ? buildEffortTooltip(effortData, false, dateText)
                : `${reviewCount} review${reviewCount !== 1 ? 's' : ''} on ${dateText}`;
        } else {
            // --- FUTURE ---
    ...
    exports.render = function (containerId, data, config) {
        ...
        // Keep preparedData at module level so createCell can read effort data without re-parsing
        state.preparedData = preparedData;

        // Initialize effort mode from config (user can toggle in-page afterwards)
        state.mode = config.effortModeEnabled ? 'effort' : 'count';
        ...
        function draw() {
                ...
                <div class="onigiri-heatmap-header">
                    ...
                    <div class="header-right">
                        ...
                        <div class="heatmap-mode-toggle">
                            <button class="mode-btn ${state.mode === 'count' ? 'active' : ''}" data-mode="count">Cards</button>
                            <button class="mode-btn ${state.mode === 'effort' ? 'active' : ''}" data-mode="effort">Effort</button>
                        </div>
            ...
            const modeToggle = container.querySelector('.heatmap-mode-toggle');
            if (modeToggle) {
