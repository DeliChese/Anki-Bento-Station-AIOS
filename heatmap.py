...
def get_heatmap_and_config():
    ...
    # Luôn đính kèm effort_by_day để JS có thể chuyển chế độ Count/Effort ngay không cần fetch lại.
    try:
        from . import habit_engine
        heatmap_data["effort_by_day"] = habit_engine.get_effort_data()
        heatmap_config["effortModeEnabled"] = conf.get("heatmapEffortMode", DEFAULTS.get("heatmapEffortMode", False))
        heatmap_config["effortWeights"] = conf.get(
            "heatmapEffortWeights",
            DEFAULTS.get("heatmapEffortWeights", {"time": 1.0, "correct": 1.0, "difficulty": 0.5}),
        )
    except Exception:
        heatmap_data["effort_by_day"] = {}
