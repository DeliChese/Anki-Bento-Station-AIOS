...
def handle_webview_cmd(handled: Tuple[bool, Any], cmd: str, context) -> Tuple[bool, Any]:
    ...
    if cmd.startswith("onigiri_set_mood:"):
        # GĐ2: mood-check 1 click từ widget Suggested Session
        try:
            mood = cmd.split(":", 1)[1]
            if mood in ("tired", "normal", "hype"):
                from . import habit_engine
                habit_engine.set_mood(mood)
                mw.col.setMod()
            # Refresh deck browser để widget cập nhật target mới
            if isinstance(context, DeckBrowser):
                context.refresh()
            return (True, None)
        except Exception:
